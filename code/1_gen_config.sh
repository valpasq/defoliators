...
echo "WRS2,sensor,date,ccland,img_obs_fn,img_pred_fn,rmse_G" > $output
for mtl in $(find ./ -name '*MTL.txt'); do
    ...
    #cc=$(grep -a -e "CLOUD_COVER" $mtl | tr -d ' ' | awk -F '=' '{ print $2 }')
    ccl=$(grep -a -e "CLOUD_COVER_LAND" $mtl | tr -d ' ' | awk -F '=' '{ print $2 }')
    
    img=./${id}/${id}_all
    pred=./$id/${id}_prediction.tif
    rmse=../../RMSE/YATSM_moths_reanalysis_${WRS}_TCG_2005-07-01.tif
    
    echo "$WRS,$sensor,$date,$ccl,$img,$pred,$rmse" >> $output
done     
